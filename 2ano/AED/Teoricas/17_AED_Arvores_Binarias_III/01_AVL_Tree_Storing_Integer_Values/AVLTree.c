...
int AVLTreeEquals(const AVLTree* root1, const AVLTree* root2) {
  ...
  return AVLTreeEquals(root1->left, root2->left) &&
         AVLTreeEquals(root1->right, root2->right);
}
...
int AVLTreeMirrors(const AVLTree* root1, const AVLTree* root2) {
  ...
  return AVLTreeMirrors(root1->left, root2->right) &&
         AVLTreeMirrors(root1->right, root2->left);
}

int AVLTreeGetNumberOfNodes(const AVLTree* root) {
  if (root == NULL) return 0;

  return 1 + AVLTreeGetNumberOfNodes(root->left) +
         AVLTreeGetNumberOfNodes(root->right);
}
