...
int BSTreeEquals(const BSTree* root1, const BSTree* root2) {
  ...
  return BSTreeEquals(root1->left, root2->left) &&
         BSTreeEquals(root1->right, root2->right);
}
...
int BSTreeGetNumberOfNodes(const BSTree* root) {
  ...
  return 1 + BSTreeGetNumberOfNodes(root->left) +
         BSTreeGetNumberOfNodes(root->right);
}

int BSTreeGetHeight(const BSTree* root) {
  if (root == NULL) return -1;

  int heightLeftSubBSTree = BSTreeGetHeight(root->left);

  int heightRightSubBSTree = BSTreeGetHeight(root->right);

  if (heightLeftSubBSTree > heightRightSubBSTree) {
    return 1 + heightLeftSubBSTree;
  }

  return 1 + heightRightSubBSTree;
}
