...
numJogos = lambda info: info[V]+info[E]+info[D]
...
pontos = lambda info: info[V]*3 + info[E]
...
def main():
    ...
    diferenca = lambda tuplo: tuplo[GM]-tuplo[GS]
    ...
    tab = sorted(tabela, key=lambda s: (pontos(s), diferenca(s)))
    printTabela(tab)
